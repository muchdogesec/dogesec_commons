...
@extend_schema_view(
    ...
    create=extend_schema(
        ...
        description=textwrap.dedent(
            ...
            * `ai_check_content` (optional, boolean): default `true`. Setting to `true` will get the AI to try and classify the text in the input to 1) determine if it is talking about threat intelligence, and 2) what type of threat intelligence it is talking about. For context, we use this to filter out non-threat intel posts in Obstracts Web and Stixify Web to reduce the cost of AI models and make them easier to search through classifications. You pass `provider:model` with this flag to determine the AI model you wish to use to perform the check. 
